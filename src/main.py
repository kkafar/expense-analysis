...
from util.lazy import LazyMethod
...
def main():
    ...
    income_monthly_net = LazyMethod(income_net_by_month, data, span)
    income_monthly_gross = LazyMethod(income_gross_by_month, data, span)
    expenses_monthly = LazyMethod(expenses_by_month, data, span)
    total_net = LazyMethod(sum_netto, data)
    total_gross = LazyMethod(np.sum, income_monthly_gross.get())
    total_expense = LazyMethod(np.sum, expenses_monthly.get())
    ...
    DataManager(Path(args.file).parent).save_categories(list(Transaction.CATEGORIES))
